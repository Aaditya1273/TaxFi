...
class VeniceClient:
    ...
    async def _chat_completion(
        ...
        """Call Venice AI chat completions endpoint."""
    ...
    async def _do_chat_completion(
        ...
        try:
            async with session.post(
                ...
                # Try to parse as JSON if response_format was requested
                if response_format:
                    try:
    ...
    def _generate_siwx_header(self) -> Optional[str]:
        """Generate SIWE header for x402 authentication."""
        if not self.wallet_key:
            return None
        import base64
        siwe = json.dumps({
        ...
        return base64.b64encode(siwe.encode()).decode()
